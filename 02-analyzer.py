...
if uploaded_file is not None:
    ...
    if st.sidebar.button("Show Analysis"):
        ...
        if not st.session_state.show_analysis and uploaded_file is None:
            st.markdown("""
            <div style="text-align: center; padding: 4rem 2rem; animation: slideInDown 0.8s ease;">
                <div style="font-size: 3.5rem; font-weight: 800; margin-bottom: 1rem; letter-spacing: -1px; background: linear-gradient(135deg, #6366f1 0%, #8b5cf6 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;">
                    💬 WhatsApp Chat Analyzer
                </div>
                <div style="text-align: center; color: #cbd5e1; font-size: 1.1rem; margin-bottom: 2rem;">
                    Transform Your Conversations into Powerful Insights
                </div>
            </div>
            """, unsafe_allow_html=True)

        if st.session_state.show_analysis and uploaded_file is not None:
            ...
            with tab1:
                ...
                ax.set_xlabel('Month with year', fontsize=12, color='#64748b', fontweight='bold')
                ...
                ax.set_xlabel('Date', fontsize=12, color='#64748b', fontweight='bold')
                ax.set_ylabel('Messages', fontsize=12, color='#64748b', fontweight='bold')
                plt.xticks(rotation='vertical', color='#64748b')
                plt.yticks(color='#64748b')
                ax.spines['top'].set_visible(False)
            ...
            with tab2:
                ...
                with col2:
                    ...
                    ax.set_xlabel('Month', fontsize=12, color='#64748b', fontweight='bold')
                    ax.set_ylabel('Messages', fontsize=12, color='#64748b', fontweight='bold')
                    plt.xticks(rotation='vertical', color='#64748b')
                    plt.yticks(color='#64748b')
                    ax.spines['top'].set_visible(False)
                ...
                ax.set_xlabel('Hour of Day', fontsize=12, color='#64748b', fontweight='bold')
                ax.set_ylabel('Day of Week', fontsize=12, color='#64748b', fontweight='bold')
                ax.set_title('Activity Heatmap', fontsize=14, color='#64748b', fontweight='bold', pad=20)
                plt.xticks(rotation=45, color='#64748b')
                plt.yticks(rotation=0, color='#64748b')
                plt.tight_layout()
                plt.tight_layout()
                st.pyplot(fig)
